...
def test_ingest_person_writes_raw_and_canonical(
...
def test_ingest_person_is_idempotent(
    db_session: Session, uow: SqlAlchemyUnitOfWork
) -> None:
    account = _account(uow)
    first = composition.processing_service.ingest_person(uow, account, _payload())
    second = composition.processing_service.ingest_person(uow, account, _payload())
...
def test_ingest_person_persists_raw_before_validation(
    db_session: Session, uow: SqlAlchemyUnitOfWork
) -> None:
    account = _account(uow)
    ...
    with pytest.raises(ValidationError):
        composition.processing_service.ingest_person(uow, account, bad)

    # Verify on a separate connection so this proves a durable commit, not just
    # presence in the service's own session identity map.
    factory = sessionmaker(
        bind=db_session.bind, autoflush=False, expire_on_commit=False
    )
    with factory() as other:
        records = other.scalars(select(models.SourceRecord)).all()
        assert len(records) == 1
        assert records[0].external_id == "people/bad"
        assert records[0].payload == bad
        assert other.scalars(select(models.Person)).all() == []


def test_ingest_person_rejects_missing_resource_name(
    db_session: Session, uow: SqlAlchemyUnitOfWork
) -> None:
    account = _account(uow)
    with pytest.raises(InvalidPayloadError):
        composition.processing_service.ingest_person(uow, account, {"names": []})
...
def test_ingest_person_changed_payload_resolves_same_person(
    ...
    account = _account(uow)
    composition.processing_service.ingest_person(uow, account, _payload())

    changed = _payload()
    changed["names"][0]["displayName"] = "Alex J. Morgan"
    changed["names"][0]["metadata"]["primary"] = False
    composition.processing_service.ingest_person(uow, account, changed)
